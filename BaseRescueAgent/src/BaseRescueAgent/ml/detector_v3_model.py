...
class DetectorV3Model:
    def __init__(self, model_path: str | Path) -> None:
        self.model_path = str(model_path)
        artifact = joblib.load(self.model_path)
        if isinstance(artifact, dict):
            self.model = artifact['model']
            self.features = list(artifact.get('features', DETECTOR_V3_FEATURES))
            self.metadata = dict(artifact.get('metadata', {}))
        else:
            self.model = artifact
            self.features = list(DETECTOR_V3_FEATURES)
            self.metadata = {}
        self.version = str(self.metadata.get('version', 'detector_v3'))
    ...
    def _phase_features(self, merged: dict[str, Any]) -> None:
