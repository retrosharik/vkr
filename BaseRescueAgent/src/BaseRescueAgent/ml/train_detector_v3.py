...
try:
    from .detector_v3_model import DETECTOR_V3_FEATURES, save_metadata_json
except ImportError:
    from detector_v3_model import DETECTOR_V3_FEATURES, save_metadata_json
...
def matrix_from_rows(rows: list[dict]) -> tuple[np.ndarray, np.ndarray]:
    x = np.asarray([[float(row.get(feature, 0.0) or 0.0) for feature in DETECTOR_V3_FEATURES] for row in rows], dtype=np.float64)
...
def sample_weights(rows: list[dict]) -> np.ndarray:
    ...
    for row in rows:
        ...
        if label == 1:
            ...
            rank_bonus = 1.0
            selected_rank = int(float(row.get('selected_rank_by_heuristic', -1) or -1))
            if selected_rank == 1:
                rank_bonus += 0.15
            weights.append(base * max(outcome_multiplier, 0.2) * rank_bonus)
        else:
            hard_negative_bonus = 1.0
            rank = int(float(row.get('heuristic_rank', 99) or 99))
            if rank <= 3:
                hard_negative_bonus += 0.2
            weights.append((total / (2.0 * neg)) * hard_negative_bonus)
...
def main() -> None:
    ...
    model = HistGradientBoostingClassifier(max_depth=6, learning_rate=0.04, max_iter=320, min_samples_leaf=18, l2_regularization=0.08, random_state=args.random_state)
    ...
    final_model = HistGradientBoostingClassifier(max_depth=6, learning_rate=0.04, max_iter=360, min_samples_leaf=18, l2_regularization=0.08, random_state=args.random_state)
    final_model.fit(x_all, y_all, sample_weight=w_all)

    artifact = {
        'model': final_model,
        'features': DETECTOR_V3_FEATURES,
        'metadata': {'version': 'detector_v3', 'group_field': args.group_field, 'report_path': str((output_dir / 'detector_v3_report.json').resolve())},
    }
    joblib.dump(artifact, output_dir / 'detector_v3.joblib')
    save_metadata_json(output_dir / 'detector_v3_report.json', report)
    save_metadata_json(output_dir / 'detector_v3_model_meta.json', artifact['metadata'])
