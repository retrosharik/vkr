...
class AStarPathPlanningCompat(PathPlanning):
    def __init__(
        self,
    ...
    def get_path(self, from_entity_id: EntityID, to_entity_id: EntityID) -> list[EntityID]:
        try:
            ...
            f_score: dict[EntityID, float] = {start: area_distance(self._world_info, start, goal)}
            while open_heap:
                ...
                for neighbor in self._graph.get(current, []):
                    tentative = g_score[current] + area_distance(self._world_info, current, neighbor)
                    if tentative < g_score.get(neighbor, float("inf")):
                        came_from[neighbor] = current
                        g_score[neighbor] = tentative
                        f_score[neighbor] = tentative + area_distance(self._world_info, neighbor, goal)
                        heapq.heappush(open_heap, (f_score[neighbor], next(serial), neighbor))
            return []
