...
from ..util.rescue_module_support import ModuleTrace, area_distance, blockades_penalty, build_area_graph, get_area_id, path_distance, reconstruct_path, road_bias_penalty


class PassableAStarPathPlanning(PathPlanning):
    def __init__(
        self,
        ...
        self._weight = float(develop_data.get_value("passable_a_star_weight", 1.15))
    ...
    def _edge_cost(self, current: EntityID, neighbor: EntityID, goal: EntityID) -> float:
        distance_value = area_distance(self._world_info, current, neighbor)
        blockade_cost = blockades_penalty(self._world_info, current) + blockades_penalty(self._world_info, neighbor)
        topology_cost = road_bias_penalty(self._world_info, neighbor, goal)
        return distance_value + blockade_cost + topology_cost
        ...
        try:
            ...
            while open_heap:
                ...
                for neighbor in self._graph.get(current, []):
                    tentative = g_score[current] + self._edge_cost(current, neighbor, goal)
                    if tentative < g_score.get(neighbor, float("inf")):
                        came_from[neighbor] = current
                        g_score[neighbor] = tentative
                        heuristic = area_distance(self._world_info, neighbor, goal)
                        priority = tentative + self._weight * heuristic
                        heapq.heappush(open_heap, (priority, next(serial), neighbor))
            return []
