...
from ..util.rescue_module_support import ModuleTrace, area_distance, cluster_entities_for_agent, count_ambulances_at, get_area_id, is_transportable_civilian, urgency_score


class CrowdAwareHumanDetector(HumanDetector):
    def __init__(
        self,
    ...
    def _score(self, civilian: Civilian) -> float:
        ...
        position_id = get_area_id(self._world_info, civilian_id)
        competitors = count_ambulances_at(self._world_info, position_id, self._agent_info.get_entity_id())
        return urgency_score(civilian) * 200.0 - distance_value * 0.15 - competitors * 5000.0
    ...
    def _select_target(self) -> Optional[EntityID]:
        ...
        position_id = get_area_id(self._world_info, target.get_entity_id())
        competitors = count_ambulances_at(self._world_info, position_id, self._agent_info.get_entity_id())
        extra = (
            f"strategy=crowd_aware distance={area_distance(self._world_info, me, target.get_entity_id()):.1f} "
            f"competitors={competitors} urgency={urgency_score(target):.2f}"
        )
