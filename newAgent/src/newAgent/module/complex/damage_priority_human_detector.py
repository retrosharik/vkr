...
class DamagePriorityHumanDetector(HumanDetector):
    def __init__(
        self,
    ...
    def calculate(self) -> HumanDetector:
        try:
            ...
            if carried is not None:
                ...
                return self
            entity = self._world_info.get_entity(self._result) if self._result is not None else None
            if self._result is not None and is_transportable_civilian(self._world_info, entity):
    ...
    def _score(self, civilian: Civilian) -> float:
        ...
        damage = float(civilian.get_damage() or 0)
        hp = float(civilian.get_hp() or 0)
        return damage * 400.0 - distance_value * 0.2 - hp * 0.01
    ...
    def _select_target(self) -> Optional[EntityID]:
        ...
        extra = f"strategy=damage_priority distance={area_distance(self._world_info, me, target.get_entity_id()):.1f} damage={target.get_damage()}"
