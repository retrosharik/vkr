...
class DistanceWeightedSearch(Search):
    def __init__(
        self,
    ...
    def _score(self, entity_id: EntityID) -> float:
        ...
        revisit_penalty = 100000.0 if entity_id in self._visited else 0.0
        return -(distance_value + revisit_penalty)
    ...
    def calculate(self) -> Search:
        try:
            ...
            self._trace.log_target("search_target", self._result, f"strategy=distance_weighted count={len(unvisited)}")
            return self
