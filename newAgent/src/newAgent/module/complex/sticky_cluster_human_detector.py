...
from ..util.rescue_module_support import ModuleTrace, area_distance, cluster_entities_for_agent, is_transportable_civilian


class StickyClusterHumanDetector(HumanDetector):
    def __init__(
        self,
        ...
        self._result_tick: int = -1
        self._lock_ticks: int = int(develop_data.get_value("sticky_human_lock_ticks", 6))
    ...
    def calculate(self) -> HumanDetector:
        try:
            self._trace.log_position_if_changed()
            now = int(self._agent_info.get_time())
            carried: Optional[Human] = self._agent_info.some_one_on_board()
            if carried is not None:
                self._result = carried.get_entity_id()
                self._result_tick = now
                self._trace.log_target("human_target", self._result, "state=carrying")
                return self
            if self._result is not None:
                entity = self._world_info.get_entity(self._result)
                if is_transportable_civilian(self._world_info, entity) and now - self._result_tick <= self._lock_ticks:
                    self._trace.log_target("human_target", self._result, f"state=locked remaining={self._lock_ticks - (now - self._result_tick)}")
                    return self
    ...
    def _select_target(self) -> Optional[EntityID]:
        ...
        target = min(
            candidates,
            key=lambda entity: (
                area_distance(self._world_info, me, entity.get_entity_id()),
                entity.get_damage() or 0,
            ),
        )
        extra = f"strategy=sticky_cluster distance={area_distance(self._world_info, me, target.get_entity_id()):.1f}"
